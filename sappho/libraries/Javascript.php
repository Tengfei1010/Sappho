...
class SH_Javascript {
	...
	public function __construct($params = array())
	{
		...
		$this->SH->load->library('javascript/'.$js_library_driver, array('autoload' => $autoload));
		// make js to refer to current library
		$this->js =& $this->SH->$js_library_driver;
	...
	function external($external_file = '', $relative = FALSE)
	{
		...
		{
			if ($this->SH->config->item('javascript_location') != '')
			{
				$this->_javascript_location = $this->SH->config->item('javascript_location');
			}
		}
		...
		{
			$str = $this->_open_script($this->SH->config->slash_item('base_url').$this->_javascript_location.$external_file);
		}
	...
	{
		$str = '<script type="text/javascript" charset="'.strtolower($this->SH->config->item('charset')).'"';
		$str .= ($src == '') ? '>' : ' src="'.$src.'">';
		return $str;
	}
