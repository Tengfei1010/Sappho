 ...
 * @link		http://codeigniter.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Client extends SH_Xmlrpc
{
	var $path			= '';
	var $server			= '';
	var $port			= 80;
	...
	function decode($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = ($this->xss_clean) ? $SH->security->xss_clean($array[$key]) : $array[$key];
				}
			}
		...
		else
		{
			...
			{
				$result = ($this->xss_clean) ? $SH->security->xss_clean($result) : $result;
			}
		}
 ...
 * @link		http://codeigniter.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Message extends SH_Xmlrpc
{
	var $payload;
	var $method_name;
	var $params			= array();
	...
	function output_parameters($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				else
				{
					...
					$array[$key] = ($key == 'bits' OR $this->xss_clean == FALSE) ? $array[$key] : $SH->security->xss_clean($array[$key]);
				}
			}
		...
		else
		{
			...
			for ($i = 0; $i < count($this->params); $i++)
			{
				...
				{
					$parameters[] = ($this->xss_clean) ? $SH->security->xss_clean($a_param) : $a_param;
				}
			}
		}
 ...
 * @link		http://codeigniter.com/user_guide/libraries/xmlrpc.html
 */
class XML_RPC_Values extends SH_Xmlrpc
{
	var $me		= array();
	var $mytype	= 0;
